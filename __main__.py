...
#print(str(RP[1]*ua_Ang)+str(" ")+str(RP[0]*ua_Ang)+str(" ")+str(Vpes*ua_eV)+"\n")
...
while Ecoll < Ecollmax:
    ...
    for ntraj in range(0,ncoup):
        ...
        if Evib==0:
            ...
            if (b<0.5):
                y[1]=sqrt(abs(2*mrbc*(Evib-Vleps-DBC)))
            else:
                y[1]=-sqrt(abs(2*mrbc*(Evib-Vleps-DBC)))
        ...
        while (t<tsup):
            ...
            if (RC[0])>(2*RABin*Ang_ua):
                ...
                print(yin1*ua_Ang,Evibtot*ua_eV)
                break    
